...
<script language="JavaScript" src="../js/validador.js"></script>
    ...
    <body>
        ...
        <div id="title"><center><spam>APURA FREQUÊNCIA INDIVIDUAL DO PARTICIPANTE</spam></center></div>
...
if ((isset($_POST["hdAcao"])) && ($_POST["hdAcao"] == "buscar")) {
	...
	$query_rsBusca = "SELECT DISTINCT e.codigo, e.nome, r.codEvento 
					  FROM evento e, registroponto r 
					  WHERE ((e.nome LIKE '%" . STRTOUPPER($_POST["txtBuscaNome"]) . "%') AND
							 (e.codigo=r.codEvento))";
...
if ((isset($_POST["hdAcao"])) && ($_POST["hdAcao"] == "inscreverManual")) {
	...
	if ($totalRows_rsBuscaParticipante > 0) {
        ...
        //VERIFICAR SE JÁ EXISTE REGISTRO DE ENTRADA E SAÍDA DO PARTICIPANTE PARA AQUELE CPF NO EVENTO
        $query_rsBuscaInscricao = 'SELECT r.codEvento, r.codParticipante 
        FROM registroponto r 
        WHERE ((r.codEvento = "'.$_POST["codEvento"].'") AND (r.codParticipante = "'.$row_rsBuscaParticipante["codParticipante"].'"))';
        ...
        if($totalRows_rsBuscaInscricao > 1){
        $query_rsInscreve = 'Call PresencaIndividual(" . $_POST["codEvento"] . " , " . $row_rsBuscaParticipante["codParticipante"] .")';
		$result = mysqli_query($conn, $query_rsInscreve);
		//CALL `PresencaIndividual`(@p0, @p1);
		//$query = "Call apurarEvento(" . $_POST["hdCodParticipante"]. ")";
		
			if ($result) {

            echo ('<div class="alert alert-success">Participante com frequência atribuída com sucesso</div>');
			}
            else{
