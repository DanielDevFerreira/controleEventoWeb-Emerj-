    ...
    if (isset($_POST["dtInicio"])!="" && ($_POST["dtFinal"])!="") {
        ...
        $queryEv = "SELECT distinct DATE_FORMAT(po.data, '%d/%m/%Y') as Data, e.nome as Evento, po.codEvento as Codigo, count(distinct r.codparticipante) as Presentes FROM evento e, porta po, registroponto r WHERE e.codigo = po.codevento and po.codevento = r.codevento and po.data BETWEEN '$dataInicio' and '$dataFinal' group by e.codigo order by po.data";
        //SELECT po.data as Data, e.nome as Evento, po.codEvento as Codigo, count(r.codparticipante) as Presentes FROM evento e, porta po, registroponto r WHERE e.codigo = po.codevento and po.codevento = r.codevento and po.data BETWEEN '2019-01-01' and '2019-03-30' group by e.codigo order by po.data
        //SELECT po.data as Data, e.nome as Evento, po.codEvento as Codigo, count(distinct r.codparticipante) as Presentes FROM evento e, porta po, registroponto r WHERE e.codigo = po.codevento and po.codevento = r.codevento and po.data BETWEEN '2019-01-01' and '2019-03-30' group by e.codigo order by po.data
    ...
    if (isset($_POST["dtInicio"])!="" && ($_POST["dtFinal"])!="") {
        ...
        $query = "SELECT count(distinct r.codparticipante) as Participantes FROM registroponto r, porta po where po.codevento = r.codevento and po.data BETWEEN '$dataInicio' and '$dataFinal'";
...
<div id="title">
    <center>
        <spam>RELATÓRIO DE EVENTOS POR PERÍODO - PARTICIPANTES PRESENTES</spam>
    </center>
</div>
    <br>
...
<?php
  ...
  if (isset($rsBusca) && ($rsBusca -> num_rows > 0)) {
  	echo("\t<tr>\n");
	echo("\t\t<td colspan=4 ><b>Total de Participantes Presentes: " . $row_rsBusca[0] . "</b></td>\n");
	echo("\t</tr>\n");
    echo("</table>");
