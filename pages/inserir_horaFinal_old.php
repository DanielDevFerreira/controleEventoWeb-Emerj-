...
$acao = NULL;
    ...
    if(isset($_POST['valorselecionado'])){
        $codEvento = $_POST['valorselecionado'];
    ...
    $resultado = 'UPDATE porta SET
		 ...
		 horaFimReal="'.$time.'" WHERE codEvento="'.$codEvento.'"';
...
?>
   ...
   function confirm() {
        ...
        function confirm2() {
         if (document.form1.codigoEvento || document.form1.data ) {
            if (document.form1.codigoEvento.value != "" || document.form1.data.value != "") {
    ...
    <div class="row">
      <div class="form-group">
          ...
          <div class="col-md-4 col-xs-6">
              ...
              <input class="form-control textoNormal" type="text" id="data" name="data" placeholder="Data do Evento" maxlength="10" onKeyPress="mascara_data(event, this.value);">
         </div>
      </div>
    </div>
  ...
  <div class="row">
	...
	$query = strtolower(trim(stripslashes("select distinct p.data, e.codigo, e.nome, e.local from porta p, evento e where e.codigo = " . $_POST["codigoEvento"] . " AND p.codevento= " . $_POST["codigoEvento"])));
	...
	$query = strtolower(trim(stripslashes("select distinct p.data, e.codigo, e.nome, e.local from porta p, evento e where data = " . $retornaDataInvertida . " and p.codevento=e.codigo order by p.data desc")));
	...
	echo "<p style='font-size:16px; text-align:center; font-family:Arial, Helvetica, sans-serif;'><strong>" . "Data pesquisada: " . $retornaDiaFormatada . "/" . $retornaMesFormatada . "/" . $retornaAnoFormatada . "</strong></p>";
	...
	for ($i=0; $i<$totalRows_rsBusca; $i++) {
		...
		for ($n=0; $n<1; $n++) {
			...
			echo("\t\t<td><input name='rdCodevento' type='radio' id='rdCodevento' value='" . $row_rsBusca[1] . "' onclick='selecionaValor(this.value);' /></td>\n");
